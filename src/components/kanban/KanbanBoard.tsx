...
import { FileText, MessageSquare, MoreVertical, ShieldCheck } from 'lucide-react';
...
export function KanbanBoard({ boardId }: KanbanBoardProps) {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    // Removendo do index original
    const sourceCards = newCards.filter(c => c.column_id === source.droppableId);
    ...
    // Persistir no backend (simplificado para o exemplo, idealmente atualiza a ordem de todos afetados)
    try {
      await supabase
        .from('kanban_cards')
    ...
    <div className="h-full w-full bg-slate-50 p-6 overflow-x-auto">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestão de Documentos e Processos Industriais Seguros</p>
        </div>
      </div>
                            ...
                            >
                               ...
                               <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                                 <div className="flex items-center space-x-2">
                                    ...
                                    <div className="flex items-center text-xs text-emerald-600 bg-emerald-50 px-2 py-1 rounded cursor-pointer hover:bg-emerald-100 transition">
